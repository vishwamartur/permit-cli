...
export default function Login() {
    ...
    useEffect(() => {
        const fetchCookie = async () => {
            ...
            }).then((response) => { setCookie(response.headers.getSetCookie()[0]); return response; })
                .then((response) => response.json());
            setState("org");
        }
    ...
    useEffect(() => {
        const fetchProjects = async () => {
            ...
            await fetch(`https://api.permit.io/v2/auth/switch_org/${activeOrg.value}`, {
                "headers": {
                    "Accept": "*/*",
                    "Origin": "https://app.permit.io",
                    "Authorization": `Bearer ${accessToken}`,
                ...
                "body": null,
                "method": "POST"
            }).then((response) => { newCookie = response.headers.getSetCookie()[0] || ''; setCookie(newCookie); return response; })
                .then((response) => response.json());
    ...
    useEffect(() => {
        const fetchEnvironments = async () => {
            const environments = await fetch(`https://api.permit.io/v2/projects/${activeProject.value}/envs`, {
                method: 'GET',
                headers: {
                    "Accept": "*/*",
                    "Origin": "https://app.permit.io",
            ...
            setEnvironments(environments.map((environment: any) => ({ label: environment.name, value: environment.id })));
        }
        if (activeProject) {
            fetchEnvironments();
        }
    ...
    return (<>
        ...
        {state === "loggingIn" && <Text><Spinner type="dots" /> Logging in...</Text>}
        {state === "org" && (orgs && orgs.length ? <>
            <Text>Select an organization</Text><SelectInput items={orgs} onSelect={handleOrgSelect} />
        </> : <Text>
            <Spinner type='dots' /> Loading Organizations
        </Text>)}
        {state === "project" && (projects && projects.length ? <>
            <Text>Select a project</Text><SelectInput items={projects} onSelect={(project) => { setActiveProject(project); setState("environment"); }} /></> : <Text><Spinner type='dots' /> Loading Projects</Text>)}
        {state === "environment" && (environments && environments.length ? <>
            <Text>Select an environment</Text><SelectInput items={environments} onSelect={(environment) => { setActiveEnvironment(environment); setState("done"); }} /></> : <Text>
            <Spinner type='dots' /> Loading Environments
        </Text>)}
        {state === "done" && activeOrg && <Text>Logged in as {activeOrg.label} with selected environment as {
